...
function my_getWXDeviceInfos() {
    ...
    WeixinJSBridge.invoke('getWXDeviceInfos', {}, function (res) {
    	...
    	for(i=0; i<res.deviceInfos.length;i++){
    		...
    		//alert(connectstr);
    			$("#devicelist").append('<td class="table"><span>设备编号：' + res.deviceInfos[i].deviceId.split("_")[2] 
				+'</span><span id="' + res.deviceInfos[i].deviceId 
				+ '">'+connectstr+'</span>');
				//+'<input type="button" value="解绑" id="'
...
//	alert("569展示前数据为---"+ strbuf);
    ...
    if ((buf[0] & 0xFF) == 0xAA && len == 1) {
        ...
        intType = 9;
        LowPow = 0;
        alert("蓝牙设备电量低，为保证正确写卡，请先对设备进行充电然后再进行相关操作");
...
//    		alert("744--qingka--"+ xiekalen[xiekaIndex][0]+ xiekalen[xiekaIndex][1]);
    		...
    		alert("密码校验失败，请联系工作人员，三次密码校验失败后卡将作废，请谨慎操作！");
    		$('#loadering').css("display","none");
    	}
	...
	$.ajax({
		...
		success:function(data,textStatus){
			...
			//alert(JSON.stringify(readData));
					// 如果是创源，进行气量取整
			//alert("1040" + readData.Factory.toLocaleLowerCase() + "/" + f_aliasname.toLocaleLowerCase() + (readData.Factory.toLocaleLowerCase() == f_aliasname.toLocaleLowerCase()));
			//alert("1041" + readData.CardID + "/" + f_cardid + (readData.CardID == f_cardid));
			//alert("1042" + ((readData.Factory.toLocaleLowerCase() == f_aliasname.toLocaleLowerCase()) && (readData.CardID == f_cardid)));
			if (readData.Err || readData.Exception)
			{
				alert("卡片错误，读取信息不正确");
				$('#loadering').css("display","none");
				return
			}
			var chazhi = (readData.CardID - 0) - (f_cardid - 0);
				//alert(chazhi);
			if (readData.Factory.toLocaleLowerCase() == f_aliasname.toLocaleLowerCase() 
					&& chazhi == 0) {
				//alert("1051" + readData.Factory);
				if(readData.Factory == "ZhenLan") { //如果是真蓝，金额输入多少就是多少
				//alert("1053" + readData.Money);
					if (readData.Money > 0) {
...
function writeServices(datas, factory, cardid, times, LjMoney) {
	var wdata;
	alert(deviceId.split("_")[2]);
	if(factory == "ZhenLan") {
		wdata = {"cardinfo": datas, "f_userid": getUrlParam('f_userid'), 
				"gas": gq, "jine": chMoney, "factory":factory, 
			"cardid": cardid, "times":times, "ljine":LjMoney, "deviceId": deviceId.split("_")[2]}
	}else if (factory == "ChuangYuan"){
		wdata = {"cardinfo": datas, "f_userid": getUrlParam('f_userid'), 
			"gas": realgqgas, "jine": realgqjine, "factory":factory, 
			"cardid": cardid, "times":times, "ljine":0, "deviceId": deviceId.split("_")[2]}
	}
	//alert(wdata);
	$.ajax({
		type:'GET',
