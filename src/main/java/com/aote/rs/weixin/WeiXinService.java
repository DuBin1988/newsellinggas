...
//			data.setTotal_fee((int) (Double.parseDouble(money) * 100));// 单位：分
	...
	public synchronized JSONObject xml(@Context HttpServletRequest request,
		...
		try {
			...
			map.put("f_coupon_fee_0", reData.getCoupon_fee_0());
			map.put("f_coupon_id_0", reData.getCoupon_id_0());
			map.put("f_coupon_fee_1", reData.getCoupon_fee_1());
			map.put("f_coupon_id_1", reData.getCoupon_id_1());
			map.put("f_coupon_fee_2", reData.getCoupon_fee_2());
			map.put("f_coupon_id_2", reData.getCoupon_id_2());
			map.put("f_coupon_fee_3", reData.getCoupon_fee_3());
			map.put("f_coupon_id_3", reData.getCoupon_id_3());
			map.put("f_coupon_fee_4", reData.getCoupon_fee_4());
			map.put("f_coupon_id_4", reData.getCoupon_id_4());
...
//				+ " group by f_userid) t group by f_userid";
	...
	public String getWrite(@Context HttpServletRequest req,
		...
		log.debug("调取卡云服务前查询用户信息为： "+ map.toString());
...
//		List list = this.hibernateTemplate.find(sql);
	...
	public JSONObject getZhye(@Context HttpServletRequest req,
		...
		System.out.println(list.get(0).toString());
		Map<String, Object> map = (Map<String, Object>) list.get(0);
		BigDecimal bg = new BigDecimal(getPregas(userid));  
		...
		obj.put("f_cardid", map.get("f_cardid"));
