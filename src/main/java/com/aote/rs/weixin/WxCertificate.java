...
//		创源使用
		...
		try {
			log.debug(price.get("f_stair1price") +"/"+ Double.valueOf((String) price.get("f_stair1price")) * 100);
			log.debug(map.get("f_metergasnums") +"/"+ (Double)map.get("f_metergasnums") *1);
			pirceint = (int)(Double.valueOf((String) price.get("f_stair1price")) * 100);
			System.out.println(pirceint);
			f_metergasnums = (int)((Double)map.get("f_metergasnums") * 1);
		} catch (Exception e) {
			System.out.println(e);
			log.debug("获取单价和累计购气量错误" + e);
			throw new RuntimeException("获取单价和累计购气量错误");
		}
		
		// 真蓝使用
		System.out.println("写卡写卡次数" + times);
		log.debug("写卡写卡次数" + times);
		System.out.println("picre="+pirceint+",f_metergasnums="+f_metergasnums);
		log.debug("单价="+pirceint+",当前表累计购气量="+f_metergasnums);
		String url = "";
		String kmm = "";
		String dqdm = "";
		log.debug("获取卡密码等参数");
		try {
			kmm = (String) map.get("kmm");
			if (kmm == null || kmm.length() == 0 ) {
				kmm = "000000";
			}
			dqdm = (String) map.get("dqdm");
			if (dqdm == null || dqdm.length() == 0) {
				dqdm = "0001";
			}
		} catch (Exception e) {
			log.debug("获取卡密码和地区代码购气量错误" + e);
		}
		log.debug("获取卡密码等参数结束");
		
//		String bjql = (String) map.get("bjql");
//		if (bjql == null || bjql.length() == 0) {
//			bjql = "1000";
//		}
//		String czsx = (String) map.get("czsx");
//		if (czsx == null || czsx.length() == 0) {
//			czsx = "9999";
//		}
//		String tzed = (String) map.get("tzed");
//		if (tzed == null || tzed.length() == 0) {
//			tzed = "0";
//		}
		JSONObject param = new JSONObject();
		try {
			if(factory.equals("ZhenLan")) {
				url = "http://127.0.0.1:8001/WriteGasCard"
						...
						+ "/" + pirceint      //旧单价，价格管理中取map.get("f_stair1price") 
						+ "/" + pirceint       //新单价，价格管理中取
						+ "/" + date         //生效日期，价格管理中取
						+ "/" + 1;         //生效标记
			...
			String result=null;
			log.debug("返回码"+httpResponse.getStatusLine().getStatusCode());
			if(httpResponse.getStatusLine().getStatusCode()==200)
			{
				entity = httpResponse.getEntity();
				result=EntityUtils.toString(entity, "UTF-8");
				log.debug("写卡云服务返回数据："+result);
				return  result;
			}else {
				throw new RuntimeException("写卡失败，返回码"+httpResponse.getStatusLine().getStatusCode());		
			}
