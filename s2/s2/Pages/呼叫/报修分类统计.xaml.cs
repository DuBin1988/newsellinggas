...
{
    public partial class 报修分类统计 : UserControl
	{
    ...
    private void so_heji(object sender, RoutedEventArgs e)
        {
            ...
            String sql = "" +
                //小区，安检情况
                        " count(t1.id) hj, t1.f_repairtype f_repairtype,count(t2.online) online,count(t3.online_zyj) online_zyj,count(h1.ywc) ywc,count(h2.dwc) dwc,count(h3.wwc) wwc,count(f1.fwmy) fwmy,count(f2.fwbmy) fwbmy,count(z1.zlmy) zlmy,count(z2.zlbmy) zlbmy" +
                        " FROM t_repairsys t1"+
                        " LEFT JOIN "+
            ...
            hejilists.HQL = "select " + sql + " where " + so.Condition + " group by t1.f_repairtype";
