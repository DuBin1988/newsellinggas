...
namespace Card
{
    public class JinKaBYNE : ICard
    {
        private static Log Log = Log.GetInstance("Card.JinKaBYNE");
        ...
        {
            Log.Debug("JinKa CheckGasCard start，实参:" + com);
            ...
            if (ret == 0)
            {
                ...
                short cs = 0;
                short bkcs = 0;
                Log.Debug("JinKa ReadGasCard start,实参:串口号：" + com);
                ret = StaticReadGasCard(0, baud, kmm, ref klx, ref kzt, cardNO, dm, yhh, tm,
                    ref ql, ref cs, ref ljgql, ref bkcs, ref ljyql, ref syql, ref bjql, ref czsx,
                    ref tzed, sqrq, ref oldprice, ref newprice, sxrq, sxbj);
              
                Log.Debug("JinKa ReadGasCard end, return:" + ret + "卡类型:" + klx);
                if (1 == klx)
                {
                    Log.Debug("此卡是金卡民用！");
                    ret = 0;
        ...
        public int ReadGasCard(short com, int baud, ref string kh, ref int ql, ref decimal money, ref short cs, ref short bkcs, ref string dqdm)
        {
            ...
            dqdm = Encoding.ASCII.GetString(dm, 2, 2);

            Log.Debug("JinKa中地区代码" + dqdm);
            return ret;
        }
