...
namespace Card
{
    public class XinAo4442 : ICard
    {
        ...
        {
            Log.Debug("format gas start");
            byte[] mm = new byte[10];
            byte[] cardNO = System.Text.Encoding.GetEncoding(1252).GetBytes(kh);
            byte[] bdqdm = new byte[4];
            int ren = StaticFormatGasCard(com, baud, mm, 1, cardNO, bdqdm);
            Log.Debug("format gas end ren = "+ren);
            return ren;
        }
        ...
        public int ReadGasCard(short com, int baud, ref string kh, ref int ql, ref decimal money, ref short cs, ref short bkcs, ref string yhh)
        {
            ...
            Log.Debug("read card end rs = "+ret);
            return ret;
        }
        ...
        {
            Log.Debug("write gas card start");
            byte[] mm = new byte[10];
            byte[] cardNO = System.Text.Encoding.GetEncoding(1252).GetBytes(kh);
            byte[] bdqdm = new byte[4];
            ...
            Log.Debug("write gas card end rs = "+ret);
            return ret;
        }
