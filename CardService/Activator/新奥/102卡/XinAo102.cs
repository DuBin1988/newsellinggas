...
namespace Card
{
    public class XinAo102 : ICard
    {
        private static Log Log = Log.GetInstance("Card.XinAo102");
        ...
        public int FormatGasCard(short com, int baud, string kmm, string kh, string dqdm)
        {
            ...
            Log.Debug("XinAo102 FormatGasCard start");
            int ren = StaticFormatGasCard(com, baud, mm, 2, cardNO, bdqdm);
            Log.Debug("XinAo102 FormatGasCard end:" + ren);
            return ren;
        }
        ...
        {
            Log.Debug("XinAo102 ReadGasCard start");
           ...
           // short bkcs = 1;
            ...
            Log.Debug("XinAo102 ReadGasCard end: kh"+kh+",gas"+ql);
            return ret;
        }
        ...
        {
            Log.Debug("XinAo102 WriteGasCard start");
            Log.Debug("Write Card Params:com--"+com+",baud--"+baud+",kmm--"+ kmm+",kh--"+  kh+",dqdm--"+ dqdm+",ql--"+ ql+",csql--"+   csql+",ccsql--"+ ccsql+",cs--"+cs+",ljgql--"+ ljgql+",bjql--"+bjql+",czsx--"+ czsx+",tzed--"+  tzed+",sqrq--"+    sqrq+",cssqrq--"+     cssqrq+",oldprice--"+     oldprice+",newprice--"+     newprice+",sxrq--"+   sxrq+",sxbj--"+  sxbj);
            byte[] mm = new byte[10];
            byte[] cardNO = System.Text.Encoding.GetEncoding(1252).GetBytes(kh);
            byte[] bdqdm = new byte[4];
            ...
            int ret = StaticWriteGasCard(com, baud, mm, 2, cardNO, bdqdm, byhh, ql, 
                cs, ljgql, bjql, czsx, tzed, bsqrq, oldprice, newprice, bsxrq, bsxbj);
            Log.Debug("XinAo102 WriteGasCard end:"+ret);
            return ret;
        }
        ...
        {
            Log.Debug("XinAo102 WriteNewCard start");
            Log.Debug("Write Card Params:com--" + com + ",baud--" + baud + ",kmm--" + kmm + ",kh--" + kh + ",dqdm--" + dqdm + ",ql--" + ql + ",csql--" + csql + ",ccsql--" + ccsql + ",cs--" + cs + ",ljgql--" + ljgql + ",bjql--" + bjql + ",czsx--" + czsx + ",tzed--" + tzed + ",sqrq--" + sqrq + ",cssqrq--" + cssqrq + ",oldprice--" + oldprice + ",newprice--" + newprice + ",sxrq--" + sxrq + ",sxbj--" + sxbj);
            ...
            short ccs = 0;
            short cbkcs = 0;
            Log.Debug("XinAo102 ReadCard start");
            int ret = ReadGasCard(com, baud, ref ckh, ref cql, ref cmoney, ref ccs, ref cbkcs, ref yhh);
            Log.Debug("XinAo102 ReadCard ret"+ret);
          
            byte[] bckh = System.Text.Encoding.GetEncoding(1252).GetBytes(ckh);
            //发卡前先格式化卡
            if (ret == 0)
            {
                Log.Debug("XinAo102 ClearCard start");
              ret = StaticFormatGasCard(com, baud, mm, 2, bckh, bdqdm);
              Log.Debug("ClearCard card end ret=" + ret);
            }
            ret = StaticWriteNewCard(com, baud, mm, 2, kzt, cardNO, bdqdm, byhh, btm, 
             ...
             Log.Debug("XinAo102 WriteNewCard ret=" + ret);
